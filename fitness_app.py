...
class FitnessApp:
    """
    Manages user registration, authentication, and
    workout logging for the Get Fit App.
    """
    ...
    def view_workout_history(self, user):
        ...
        for workout in user.workouts:
            print(f"Date: {workout.date}, Duration: {workout.duration}"
                  f"min, Exercises: {', '.join(workout.exercises)}")
    ...
    def track_goal_progress(self, user):
        ...
        workouts_this_week = [
            ...
            if datetime.strptime(w.date, "%Y-%m-%d").isocalendar()[1]
            == current_week
        ]
